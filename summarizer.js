// ─────────────────────────────────────────────────────────
// summarizer.js — Google Gemini API summarization engine
// ─────────────────────────────────────────────────────────
...
- Tone: dry, matter-of-fact, human. Not robotic. Match the gravitas of the original story but strip any bias or editorializing.
...
async function summarizeStory(storyGroup, categoryKey) {
  ...
  // Build context from all sources in the group
...
Summarize this into the required format. Remember: 40-75 words max total, neutral tone, no hooks. Return only valid JSON.`;
  ...
  try {
    ...
    if (!parsed.headline || !parsed.coreSummary) {
      console.warn('  [WARN] Missing required fields in AI response');
      return null;
    }
